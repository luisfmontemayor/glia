...
pub fn queue_telemetry(json_payload: String, url: String, timeout: f64) -> PyResult<()> {
    ...
    match result {
        Ok(inner) => inner.map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e)),
        Err(_) => Err(pyo3::exceptions::PyRuntimeError::new_err("Rust panicked during queue_telemetry")),
    }
}
...
pub fn flush_queue() -> PyResult<PyFlushSummary> {
    ...
    match result {
        ...
        Err(_) => Err(pyo3::exceptions::PyRuntimeError::new_err("Rust panicked during flush_queue")),
    }
}
...
pub fn trigger_panic() -> PyResult<()> {
    ...
    match result {
        Ok(_) => Ok(()),
        Err(_) => Err(pyo3::exceptions::PyRuntimeError::new_err("Intentional Rust panic caught at FFI boundary")),
    }
}
