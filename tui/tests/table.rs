...
fn should_highlight_column_in_column_mode() {
    ...
    assert!(highlighted_cells.len() > 0, "No cells were highlighted in column mode");
}
    ...
    for y in 0..buffer.area.height {
        for x in 0..buffer.area.width {
            ...
            if cell.symbol() == "j" && buffer.get(x+1, y).symbol() == "o" && buffer.get(x+2, y).symbol() == "b" {
                job1_found = true;
                assert!(!cell.style().add_modifier.contains(Modifier::REVERSED), "Focused cell should NOT be REVERSED");
