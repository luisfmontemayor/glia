...
def push_telemetry(
    ...
    """
    - Auto-config: Reads API_URL env var if api_url is not provided.
    - Suppress Errors: Returns False on failure instead of crashing (Fail Silent).
    """
    target_url: str | None = api_url or os.getenv("API_URL")
    if not target_url:
        logger.warning("[Glia] No API_URL configured. Telemetry dropped.")
        return False
