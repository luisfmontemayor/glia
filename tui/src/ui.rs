...
fn render_metric_chart(f: &mut Frame, app: &App, area: Rect) {
    let data: Vec<u64> = match app.metric {
        ...
        Metric::CpuTime => app
            .jobs
            .iter()
            .map(|j| (j.cpu_time_sec * 1000.0) as u64)
            .collect(),
        ...
        Metric::Throughput => app.jobs.iter().map(|_| 1u64).collect(),
        Metric::SuccessRate => app
            .jobs
            .iter()
            .map(|j| if j.exit_code_int == 0 { 100u64 } else { 0u64 })
            .collect(),
    };
